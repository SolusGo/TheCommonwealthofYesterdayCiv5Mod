...
('TXT_KEY_CIV5_COMMONWEALTH_TEXT_5',
 ...
 || '[NEWLINE][NEWLINE]' ||
 'Adjacent Old Friends may also speak to one another. Each eligible pair has an 8% chance per turn to produce an exchange, increased during a Reminiscence and near a Childhood Bedroom. Era changes, upgrades, close calls, victories, Reminiscence activations, and reunions after a long separation enter persistent event queues with a higher chance to occur. Global, pair, and dialogue-history cooldowns keep the system from becoming repetitive, and conversations can be disabled in the Ledger.'),
