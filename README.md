...
**Repository version: 1.0.68** — this documentation version is independent of the in-game ModBuddy version, which intentionally remains `1`.
...
The conversation library contains 136 exchanges. Its everyday chatter favors loose childhood banter, gaming slang, friendly roasting, and the occasional ridiculous argument, while reunions, Reminiscences, and dangerous moments retain the warmer emotional lines. Forty additional exchanges cover homework, sleepovers, snacks, split-screen disputes, fake rage quits, and technology-aware jokes. Era-scoped chatter changes as the Commonwealth advances: pointy sticks and newly invented wheels give way to castles and crossbows, then trains and tanks, and finally unreliable Wi-Fi, suspicious robots, and spaceships. Dialogue is stored separately from the Lua logic in `Data/CommonwealthConversations.xml`. The file defines the custom `Commonwealth_Conversations` database table, which Lua reads directly so its descriptive text IDs remain distinct; exchanges can be added or revised without editing the conversation system itself. Lines can use `{SELF}`, `{OTHER}`, `{LOCATION}`, `{ERA}`, `{FORM}`, and `{OTHER_FORM}` tokens for contextual details.
